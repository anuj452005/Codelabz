...
// Upload any media file (image, video, or document)
export const uploadTutorialMedia =
  (owner, tutorial_id, file, mediaType) =>
  async (firebase, firestore, dispatch) => {
    try {
      if (!tutorial_id) {
        alert("Please create the tutorial first, then upload media.");
        return;
      }
      dispatch({ type: actions.TUTORIAL_MEDIA_UPLOAD_START });

      // Reuse the same storage path pattern as uploadTutorialImages
      const type = await checkUserOrOrgHandle(owner)(firebase, firestore);
      const storagePath = `tutorials/${type}/${owner}/${tutorial_id}/${file.name}`;
